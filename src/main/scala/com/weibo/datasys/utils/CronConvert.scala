...
object CronConvert {
  ...
  /**
    ...
    * @return
    */
  def convert(cron: String): (Long, String) = {
    implicit val _debug_mode: Boolean = true
//    MyLogging.debug(s"raw cron string = $cron")

    var result: (Long, String) = (0L, "")
    val arr = cron.split("\\s+")
    if (arr.size > 6) {
      val List(min, hour, day, dayOfMonth, weekday) = arr.toList.take(5)
//      MyLogging.debug(s" min = ${String.format("%-10s", min)} " +
//        s"hour = ${String.format("%-10s", hour)} " +
//        s"day = ${String.format("%-10s", day)} " +
//        s"\tdayOfMonth = $dayOfMonth\tweekday = $weekday ")
      ...
      if (wk_list.nonEmpty) {
        ...
        val mm: Int = cron_map
          ...
          .getOrElse(0)
        wk_list.map{ wd =>
          val x = getLastWeekDay(wd, hh, mm)
          MyLogging.debug(s" x = $x interval = $interval")
        }
      } else {
            ...
            cron_map.get(CRON_KEY_MIN).foreach { mm =>
              mm match {
                ...
                case _ =>
                  ...
                  if (mins.size > 1) {
                    mins map { mmm =>
                      interval = SECONDS_OF_HOUR
                      val x = getLastHourMin(mmm)
                      MyLogging.debug(s" x = $x interval = $interval")
                    }
